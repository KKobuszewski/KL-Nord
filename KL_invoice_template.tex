...
\begin{document}
	...
	\begin{tabular*}{0.9\linewidth}{@{\extracolsep{\fill}} llllll}
		...
		&  &  &  &  & \tabularnewline
		&  &  &  & data sprzeda{\.z}y & {{ date }} \tabularnewline
	...
	%~\\
	\begin{multicols}{2}
		Bank: PKO BP Oddz.1 Slupsk, ul. Wazow 5
		...
		P{\L}ATNO{\'S}{\'C}: 60 DAYS FROM THE DATE
	...
	\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}|ll|ll|ll|}
		\hline 
		\multicolumn{1}{l}{} & \multicolumn{1}{l}{} &  & \multicolumn{1}{l}{} &  & \multicolumn{1}{l}{}\tabularnewline[-1em]
		\hline
		\\[-1em]
		l.P. USŁUGA  &  & WARTO{\'S}{\'C} NETTO &  & WARTO{\'S}{\'C} BRUTTO & \tabularnewline
		...
		faktur\k{a} nr {{ invno }} &  &  &  &  & \tabularnewline
		...
		No. {{ invno }} fr {{ date }} &  &  & {{ value }} &  & {{ value }}\tabularnewline
		\hline
		\\[-1em]
	...
	\begin{tabular}{ll}
		\textbf{DO ZAP{\L}ATY:} & \textbf{ {{ value }}~EUR}\tabularnewline
		\textbf{TO BE PAID:} & \tabularnewline
		& \tabularnewline
		S{\L}OWNIE: & {{ valuesaypl }}~EUR\tabularnewline
		SAY:&{{ valuesayen }}~EUR\tabularnewline
	\end{tabular}
	...
	Nabywca us{\l}ugi jest p{\l}atnikiem podatku VAT „odwrotne obci\k{a}{\.z}enie”
